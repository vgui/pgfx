...
public:
    ...
    Win32Window(const char* title, int x, int y, int width, int height)
    {
...
protected:
    ...
    static LRESULT StaticProcessEvent(HWND handle, UINT msg, WPARAM wparam, LPARAM lparam)
    {
        ...
        case WM_CREATE:
        {
            ...
            if(win_ptr)
                ...
                throw ExceptionInfo << "Window pointer is NULL in StaticProcessEvent function.";
    ...
    virtual void Create(const char* title, int x, int y, int width, int height)
    {
        ...
        if(!s_windows.size() && !::RegisterClassExW(&wndclassex))
            throw ExceptionInfo << "Can't register Win32/Win64 window class.";
        ...
        m_handle = ::CreateWindowExW(0, s_classname.c_str(), wtitle.c_str(), WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS,
                                     0, 0, 100, 100, nullptr, nullptr, s_hinstance, this);
        ...
        ::MoveWindow(m_handle, x, y, width, height, TRUE);//Need this call for generate first WM_PAINT message.
    }
